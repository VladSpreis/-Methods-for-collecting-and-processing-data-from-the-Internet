...
class CastoramaSpider(scrapy.Spider):
    ...
    def parse_add(self, response:HtmlResponse):
        ...
        # price = response.xpath("//div[@class ='add-to-cart__price js-fixed-panel-trigger']//span[@class='price']//text()").get()
        # url = response.url
        # photos = response.xpath("//li[contains(@class, 'top-slide swiper-slide')]/div/img/@data-src").getall()
        # yield CastparserItem(title=title, price=price, url=url, photos=photos)
